# Reference Code for Swipeable Dock Implementation

## Installation Requirements

```bash
pnpm add motion react-icons tailwind-merge clsx vaul
```
...
## Source Code (components/dynamic-toc-filter.tsx)
...
This code provides the foundation for the swipeable sharing dock. Key elements to adapt:

1. **Interface**: Change `TOC_INTERFACE` to `SHARING_INTERFACE` with sharing-specific properties
2. **Progress Component**: Replace with sharing trigger button
3. **Text Component**: Replace with sharing title and icon
4. **Items Component**: Replace with sharing options (WhatsApp, LinkedIn, etc.)
5. **Scroll Progress**: Replace with sharing-specific functionality

The animation patterns, styling, and interaction model should remain the same for consistency.